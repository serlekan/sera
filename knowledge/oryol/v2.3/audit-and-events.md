# Oryol Core — Audit, Outbox & Event-Driven Architecture v2.3

**Status**: PROPOSED ARCHITECTURE BASELINE (v2.3) — Subject to Independent Architecture Review  
**Revision Scope**: Security Policy, IP Allowlist, and Service Principal Role Audit Actions (ADR-001, ADR-002)
...
All security-critical operations require the **business state mutation**, **version increment**, and the corresponding **audit event insert** to execute in the **same atomic D1 transaction**:
- Membership revocation / role change (`core.rbac.role_assigned`, `core.rbac.role_removed`)
- Service principal role assignment / removal (`core.rbac.service_principal_role_assigned`, `core.rbac.service_principal_role_removed`)
- Security policy updates (`core.security_policy.updated`)
- IP allowlist entry additions, status updates, and deletions (`core.ip_allowlist.entry_added`, `core.ip_allowlist.entry_updated`, `core.ip_allowlist.entry_removed`)
