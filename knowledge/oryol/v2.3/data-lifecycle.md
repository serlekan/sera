# Oryol Data Lifecycle & Deletion Architecture v2.2
...
Architecture v2.2 replaces inaccurate claims of instant cryptographic erasure with a realistic, multi-phase deletion pipeline:
...
| **Audit Logs** | Append-Only Privacy Overlay (`audit_redactions`) / Legal Hold. | **Permanently Retained (No Physical Purge / Zero In-Place Updates)**. Read path applies redactions without mutating raw audit events. | Never cascade-deleted with organization purge. D1 triggers enforce append-only immutability. |
