...
## 8. Organization Security Policy Dynamic Enforcement (ADR-001 — F-4)
...
### 8.2 Dynamic Session Timeout Policies (A2-2)
While `account_sessions` tracks the authenticated human principal at the identity layer, timeout policies are authoritatively configured and enforced per organization via `organization_security_policies`:
- **Organization-Scoped Token Lifetime**: When issuing or validating an organization-scoped access token, the target organization's `session_idle_timeout_seconds` (default: 86,400s / 24h; min: 300s) and `session_absolute_timeout_seconds` (default: 604,800s / 7d; min: 3,600s) directly govern token TTL.
- **Multi-Organization Resolution (Strictest Tenant Enforcement)**: A principal belonging to multiple organizations is governed by the specific active organization's policy for requests to that organization. If `(datetime('now') - account_sessions.last_active_at)` exceeds the active organization's `session_idle_timeout_seconds`, the access token for that organization is rejected with `DENY(SESSION_IDLE_TIMEOUT)`, requiring session refresh or re-authentication before proceeding.
- **Global Session Boundary**: Refresh token expiration is bounded by the minimum of `+30 days` and the most restrictive active organization `session_absolute_timeout_seconds`. Once exceeded, full re-authentication is mandatory.
