...
- **Absolute Timeout**: Evaluated against `account_sessions.created_at` and `organization_security_policies.session_absolute_timeout_seconds` (default: 604,800s / 7d; min: 3,600s). Refresh token expiration is bounded by `MIN('+30 days', session_absolute_timeout_seconds)`. Once exceeded, re-authentication is mandatory.
